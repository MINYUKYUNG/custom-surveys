...
import { addOption, deleteOption, editOptionText, changeOptionOrder } from '../../store/questions';
import {
...
function TextOrOption({ id, type, options }: quesType.QuestionsGuard) {
  ...
  } else if (quesConst.ESSAY_TYPE === type) {
    return (
      <input 
        type="text" 
  ...
  const showOptions = options.map((option, index) => {
    const whichType = () => {
      ...
      else if (quesConst.CHECKBOX_TYPE === type) return <MdCheckBoxOutlineBlank size="1.5em" />;
      else if (quesConst.DROPDOWN_TYPE === type) return <MdOutlineArrowDropDownCircle size="1.5em" />;
    };
